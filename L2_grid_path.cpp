    ...
    while (!pq.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (isValid(newX, newY, n)) {
                ...
                int g_new = temp->state_array[temp->x][temp->y].first + 1;
                int h_new = manhattan_dist(newX, newY, destX, destY);

                new_board[newX][newY].first = g_new;
                new_board[newX][newY].second = h_new;
...
int main() {
    ...
    for (int a = 0; a < n; a++) {
        for (int b = 0; b < n; b++) {
            if (grid[a][b].second != INT_MAX) { // skip water
